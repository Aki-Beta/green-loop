// frontend/js/views/certificado.js - Vista Certificado (detalle + descarga PDF)

function getFactorCalidad(calidad) { return { A: 1.2, B: 1.0, C: 0.7 }[calidad] || 1.0; }
function getFactorZona(zona) { return { urbana: 1.0, rural: 1.3 }[zona] || 1.0; }

async function renderCertificado(params) {
    ...
    try {
        const [carga, incentivo] = await Promise.all([
            api.cargas.obtener(cargaId),
            api.cargas.obtenerIncentivo(cargaId)
        ]);
        ...
        app.innerHTML = `
            <div class="max-w-md mx-auto text-center py-12">
                ...
                <button onclick="navigate('/')" class="bg-primary-600 text-white px-5 py-2 rounded-lg font-medium hover:bg-primary-700 transition">Volver</button>
            </div>
        `;
    }
}
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                <div>
                    <h1 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
                        <i class="fas fa-certificate text-primary-600"></i> Certificado de Gestión
                    </h1>
                    <p class="text-gray-600 mt-1">Consecutivo: <span class="font-mono font-medium">${cert.consecutivo}</span></p>
                </div>
                <div class="flex gap-3">
                    <button onclick="downloadPDF(${cargaId})" class="bg-primary-600 hover:bg-primary-700 text-white px-5 py-2.5 rounded-lg font-medium flex items-center gap-2 transition">
                        <i class="fas fa-download"></i> Descargar PDF
                    </button>
                    <button onclick="navigate('/')" class="px-5 py-2.5 border border-gray-300 text-gray-700 rounded-lg font-medium hover:bg-gray-50 transition">Volver</button>
                </div>
            </div>
                ...
                <div class="p-6 space-y-5">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-gray-800 font-medium">${carga.usuario}</p>
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-500 uppercase">Ruta / Municipio</label>
                            <p class="text-gray-800 font-medium">${carga.camion.ruta?.nombre || '-'} / ${carga.camion.ruta?.municipio || '-'}</p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div class="bg-gray-50 rounded-lg p-4">
                            ...
                            <p class="text-gray-800 font-medium capitalize">${carga.tipo_residuo}</p>
                        </div>
                        <div class="bg-gray-50 rounded-lg p-4">
